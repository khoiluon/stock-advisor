...
# ==============================================================================
# ML Data — Stock & StockData riêng cho ML pipeline (tách biệt ADMRS)
#
# Mục đích:
#   - Lưu adjusted OHLCV (adj_open/high/low/close/volume) đã tính sẵn
#   - run_ml_predictions đọc từ đây, KHÔNG đụng vào Stock/StockData của ADMRS
#   - Tránh xung đột dữ liệu khi ADMRS và ML chạy song song
#   - Sau này có thể sync từ ADMRS hoặc load thẳng từ parquet
# ==============================================================================

class MLStock(models.Model):
    """
    Bản sao thông tin cơ bản cổ phiếu dành riêng cho ML pipeline.
    Đồng bộ từ Stock (ADMRS) qua management command `sync_ml_stocks`,
    hoặc load thẳng từ stock_metadata.parquet.
    """
    ticker = models.CharField(max_length=20, unique=True, primary_key=True)
    company_name = models.CharField(max_length=255, null=True, blank=True)
    exchange = models.CharField(max_length=10, null=True, blank=True)
    industry = models.CharField(max_length=255, null=True, blank=True)
    stock_type = models.CharField(max_length=20, null=True, blank=True)
...
class MLStockData(models.Model):
    """
    OHLCV dành riêng cho ML pipeline.
    Lưu raw data + adj_close. Các giá trị adjusted (adj_open/high/low/volume)
    được tính trong Python khi cần, không lưu vào DB.
    """
    stock = models.ForeignKey(MLStock, on_delete=models.CASCADE, related_name='ml_data_points')
    date = models.DateField()
    ...
    volume = models.BigIntegerField()
    adj_close = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
...
class MLPrediction(models.Model):
    ...
    stock = models.ForeignKey(MLStock, on_delete=models.CASCADE, related_name='ml_predictions')
...
class AnomalyAlert(models.Model):
    ...
    stock = models.ForeignKey(MLStock, on_delete=models.CASCADE, related_name='anomaly_alerts')
