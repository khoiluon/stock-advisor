...
const ChatbotWindow = () => {
    ...
    const handleSendMessage = async (e) => {
        ...
        const userMessage = { sender: 'user', content: inputMessage, id: `user-${Date.now()}` };
            ...
            <div className="flex-1 p-4 overflow-y-auto">
                {messages.length === 0 && !isLoading && (
                    <div className="text-center text-gray-400 my-auto h-full flex items-center justify-center">
                        <p>Chào bạn! Hãy hỏi tôi về một mã cổ phiếu cụ thể nhé.<br/>Ví dụ: "Phân tích cổ phiếu FPT" hoặc "HPG hôm nay thế nào?"</p>
                    </div>
                )}
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex my-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        >
                            ...
                            <div className="prose prose-invert" dangerouslySetInnerHTML={{ __html: marked(msg.content) }} />
                        </div>
                    </div>
                ))}
                {isLoading && (
            ...
            <div className="p-4 border-t border-gray-700">
                <form onSubmit={handleSendMessage}>
                    <div className="flex items-center bg-[#232e43] rounded-lg">
                        ...
                        />
                        <button type="submit" disabled={isLoading} className="p-3 text-blue-400 hover:text-blue-300 disabled:text-gray-500">
