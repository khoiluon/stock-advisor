...
const loadOptions = (inputValue, callback) => {
	...
	const searchUrl = `${import.meta.env.VITE_API_URL}/stocks/search/?q=${encodeURIComponent(inputValue)}`;
	...
	const handleToggleFavorite = async () => {
		...
		try {
			if (!isFavorite) {
				...
				setIsFavorite(true);
				toast.success(
					`${stockInfo.ticker || ticker} was added to your favorites!`,
				);
			} else {
				...
				const favItem = res.data.find(
					(fav) =>
						(fav.stock && fav.stock.ticker) === (stockInfo.ticker || ticker),
				);

				if (favItem) {
					await axios.delete(
						`${import.meta.env.VITE_API_URL}/watchlist/${favItem.id}/`,
						{ headers: { Authorization: `Token ${token}` } },
					);
					setIsFavorite(false);
					toast.error(
						`${stockInfo.ticker || ticker} was removed from your favorites.`,
					);
				}
			}
		} catch (err) {
		...
		const fetchData = async () => {
			...
			try {
				...
				const [stockRes, infoRes] = await Promise.all([
					axios.get(
						`${import.meta.env.VITE_API_URL}/stock-data/?ticker=${ticker}`,
						{ headers: { Authorization: `Token ${token}` } },
					),
					axios.get(`${import.meta.env.VITE_API_URL}/stocks/${ticker}/`, {
						headers: { Authorization: `Token ${token}` },
					}),
				]);
		...
		<div>
			<main className="px-10 py-8">
				...
				<div className="bg-[#1a2332] p-4 rounded-xl mb-8 flex items-center justify-between">
					<div className="flex items-center space-x-4">
						<label className="text-gray-300 font-semibold shrink-0">
							Stock Ticker:
						</label>
						<AsyncSelect
							cacheOptions
