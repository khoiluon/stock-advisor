...
const Favorites = () => {
	...
	const fetchFavorites = async () => {
		...
		const res = await axios.get(`${import.meta.env.VITE_API_URL}/watchlist/`, {
	...
	const handleDelete = async (id) => {
		const token = localStorage.getItem("token");
		await axios.delete(`${import.meta.env.VITE_API_URL}/watchlist/${id}/`, {
			headers: { Authorization: `Token ${token}` },
		});
		...
		<div>
			...
			<div className="w-full max-w-3xl mx-auto">
				{favorites.map((fav) => (
					<div
						key={fav.id}
						className="bg-[#232e43] rounded-xl shadow-lg p-6 flex items-center justify-between mb-6"
					>
